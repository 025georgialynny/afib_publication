...
file_process = function(number){
  if (VERSION == "MIT"){
    read.csv(paste("/rhome/jlaws011/AFib/Data/", number, 
                 ".csv", sep = ""), header = T)
  }else if (VERSION == "single_2017"){
    read.csv(paste("/rhome/jlaws011/AFib/Data/Training_Set/", number, 
                   ".csv", sep = ""), header = T)
  }else{
    read.csv(paste("/rhome/jlaws011/AFib/Data/Training_Set/", number, 
...
rel_freq = function(num, VERSION){
  ...
  if (begin != T){
    ...
    write.csv(rel_freq_info, paste("/rhome/jlaws011/AFib/Data/", subjects[num], VERSION,
                                 "tran_mat.csv", sep = ""))
